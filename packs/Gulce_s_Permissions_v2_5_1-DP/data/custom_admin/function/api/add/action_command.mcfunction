...
$data modify storage glc:data actions append value {id:"$(id)",type:'command',params:{command:"$(command)"},player:"$(selector)"}

$execute if entity @s[tag=glc.lang_tr] run tellraw @s [{text:"[GULCE] ",color:"gold",bold:true},{text:"✅ Komut eklendi: ",color:"green"},{text:"$(id)",color:"yellow"}]
$execute if entity @s[tag=glc.lang_en] run tellraw @s [{text:"[GULCE] ",color:"gold",bold:true},{text:"✅ Komut eklendi: ",color:"green"},{text:"$(id)",color:"yellow"}]
