...
$data modify storage glc:data actions append value {id:"$(id)",type:'time',params:{mode:"$(mode)",value:"$(value)"},player:"$(selector)"}

$execute if entity @s[tag=glc.lang_tr] run tellraw @s [{text:"[GULCE] ",color:"gold",bold:true},{text:"✅ Zaman eylemi eklendi: ",color:"green"},{text:"$(id)",color:"yellow"}]
$execute if entity @s[tag=glc.lang_en] run tellraw @s [{text:"[GULCE] ",color:"gold",bold:true},{text:"✅ Time action added: ",color:"green"},{text:"$(id)",color:"yellow"}]
