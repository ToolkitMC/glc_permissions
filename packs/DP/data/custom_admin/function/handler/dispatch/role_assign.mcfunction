...
execute if data storage glc:dispatch {flat:{role:"owner"}} unless entity @s[tag=gulce_owner] run return 0

# Rol dağıtımı — flat storage ile çağır
execute if data storage glc:dispatch {flat:{role:"admin"}} run function custom_admin:handler/dispatch/role_assign_run with storage glc:dispatch flat

execute if data storage glc:dispatch {flat:{role:"mod"}} run function custom_admin:handler/dispatch/role_assign_run with storage glc:dispatch flat

execute if data storage glc:dispatch {flat:{role:"vip"}} run function custom_admin:handler/dispatch/role_assign_run with storage glc:dispatch flat

execute if data storage glc:dispatch {flat:{role:"player"}} run function custom_admin:handler/dispatch/role_assign_run with storage glc:dispatch flat

execute if data storage glc:dispatch {flat:{role:"owner"}} run function custom_admin:handler/dispatch/role_assign_run with storage glc:dispatch flat
