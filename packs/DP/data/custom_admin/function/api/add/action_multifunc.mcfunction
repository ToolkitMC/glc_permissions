...
$data modify storage glc:data actions append value {id:"$(id)",type:'multi_fn',params:{funcs:$(funcs)},player:"$(selector)"}

$execute if entity @s[tag=glc.lang_tr] run tellraw @s [{text:"[GULCE] ",color:"gold",bold:true},{text:"✅ Fonksiyonlar eklendi: ",color:"green"},{text:"$(id)",color:"yellow"},{text:" $(selector)",color:"aqua"}]
$execute if entity @s[tag=glc.lang_en] run tellraw @s [{text:"[GULCE] ",color:"gold",bold:true},{text:"✅ Fonksiyonlar eklendi: ",color:"green"},{text:"$(id)",color:"yellow"},{text:" $(selector)",color:"aqua"}]
