...
# 1. YETKİ KONTROLÜ
execute unless entity @s[tag=gulce_owner] run tellraw @s {"text":"❌ Bu panel sadece OWNER yetkisine özeldir!","color":"red"}
...
# --- Diğer ---
data modify storage glc:ui ui.actions append value {label:"Eylem Çalıştırıcı",action:{type:"minecraft:show_dialog","dialog":{"type": "minecraft:multi_action","title":"Eylem çalıştır","inputs":[{"type":"minecraft:text","key":"id","label":"ID","label_visible":true,"max_length":2000000000},{"type":"minecraft:boolean","key":"name","label":"Tüm oyuncular için çalıştır","initial":true,"on_true":"@a","on_false":"@s"}],"can_close_with_escape":true,"pause":false,"after_action":"close","actions":[{"label":"Çalıştır","action":{"type": "minecraft:dynamic/run_command","template":"/execute as $(name) run function custom_admin:handler/execute/action {\"id\":\"$(id)\"}"}}]}}}
